...
const AGENTS = [
  {
    ...
    desc: "Structured sessions that build confidence, teamwork, and responsibility through practical guidance and mentorship.",
    stats: [{ v: "Mentoring", l: "support" }, { v: "Growth", l: "pathways" }],
    img: "/images/malay-group-2.png",
  },
  {
    label: "VOLUNTEER",
    title: "Volunteer activities",
    desc: "Hands-on service opportunities that encourage young people to show up for communities with consistency and care.",
    stats: [{ v: "Service", l: "culture" }, { v: "Action", l: "focus" }],
    img: "/images/malay-group-3.png",
  },
  {
    label: "OUTREACH",
    title: "Community outreach initiatives",
    desc: "Neighbourhood engagement, partnership visits, and local support efforts that connect youth with real community needs.",
    stats: [{ v: "Local", l: "partnerships" }, { v: "Shared", l: "impact" }],
    img: "/images/malay-4.png",
  },
  {
    label: "WORKSHOPS",
    title: "Training & workshops",
    desc: "Practical learning spaces for communication, leadership values, and organising effective youth-led programmes.",
    stats: [{ v: "Skills", l: "building" }, { v: "Values", l: "based" }],
    img: "/images/malay5.png",
  },
]
...
function Tag({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-flex items-center px-3 py-1 rounded-full text-[11px] tracking-widest font-sans text-white/55 bg-white/[0.05]">
      {children}
    </span>
  )
}
      ...
      {AGENTS.map((agent, i) => {
        ...
        return (
          ...
          >
            ...
            >
              <div className="group relative bg-[#0b0b0b] rounded-2xl border border-white/[0.08] overflow-hidden cursor-pointer">
                ...
                {agent.img && (
                  <div className="hidden md:block absolute inset-y-0 right-0 w-1/2 pointer-events-none">
                    ...
                    />
                      ...
                      style={{
                        background: "linear-gradient(to right, #0b0b0b 0%, transparent 55%)",
                      }}
                    />
                  </div>
                )}
                ...
                >
                  <div className="md:max-w-[60%]">
                    ...
                    <p className="text-sm text-white/65 leading-relaxed mb-8">{agent.desc}</p>
                  </div>
                  <div className="flex gap-8 pt-6 border-t border-white/[0.08]">
                    {agent.stats.map(s => (
                      <div key={s.l}>
                        <div className="text-2xl font-light text-white">{s.v}</div>
                        <div className="text-[11px] text-white/45 tracking-widest mt-0.5">{s.l}</div>
                      </div>
                    ))}
                  </div>
                </div>

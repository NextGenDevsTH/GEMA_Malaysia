    ...
    >
      <div className="glass-surface relative z-[70] mx-auto flex w-full max-w-[1440px] items-center justify-between gap-3 rounded-full px-3 py-2.5 shadow-[0_18px_70px_rgba(0,0,0,0.32)] ring-1 ring-white/8 sm:gap-4 sm:px-4">
        ...
        <button
          ...
          className="relative z-[65] rounded-full border border-white/10 bg-white/6 px-4 py-2 text-sm text-white/86 transition hover:bg-white/10 xl:hidden"
        >
        ...
        {mobileOpen ? (
          <>
            ...
            <motion.nav
              ...
              className="fixed inset-x-3 top-[5.75rem] z-[60] max-h-[calc(100dvh-7rem)] overflow-y-auto rounded-[28px] border border-[#ded5f3] bg-[#ffffff] p-3 text-[#1a1a1a] shadow-[0_24px_80px_rgba(20,13,38,0.28)] xl:hidden"
            ...
            >
              <div className="grid gap-2">
                {primaryLinks.map((page) => (
                  <Link
                    key={page.href}
                    href={page.href}
                    className={`rounded-2xl px-4 py-3 text-sm transition hover:bg-[#f3efff] ${
                      isActive(pathname, page.href) ? "bg-[#efe8ff] text-[#4c248f]" : "text-[#2d2d2d]"
                    }`}
                  >
                    {page.label}
                  </Link>
                ...
                {navGroups.map((group) => (
                  <div key={group.label} className="rounded-[22px] border border-[#e8e0f7] bg-[#fbfaff] p-2">
                    <Link
                      href={group.href}
                      className={`flex items-center justify-between rounded-2xl px-3 py-2.5 text-sm transition hover:bg-[#f3efff] ${
                        isGroupActive(pathname, group) ? "text-[#4c248f]" : "text-[#1a1a1a]"
                      }`}
                    >
                      <span>{group.label}</span>
                      <span className="text-[10px] uppercase tracking-[0.26em] text-[#6f647f]">{group.kicker}</span>
                    </Link>
                    <div className="mt-1 grid grid-cols-1 gap-1 sm:grid-cols-2">
                      {group.links.map((link) => (
                        <Link
                          ...
                          target={link.href.startsWith("http") ? "_blank" : undefined}
                          rel={link.href.startsWith("http") ? "noreferrer" : undefined}
                          className={`rounded-2xl px-3 py-2.5 text-sm transition hover:bg-[#f3efff] ${
                            isActive(pathname, link.href) ? "bg-[#efe8ff] text-[#4c248f]" : "text-[#3d3748]"
                          }`}
                        >
                          {link.label}
                        </Link>
              ...
              <div className="mt-3 grid gap-2">
                ...
                <Link
                  href="/contact"
                  className="rounded-2xl border border-[#d9cff0] bg-[#ffffff] px-4 py-3 text-center text-sm font-semibold text-[#5f35b5]"
                >
                  Contact
                </Link>
              </div>
